...
void loadConfiguration(){
  ...
  DynamicJsonDocument device(4096),server(4096);
  if(loadJson("/device.json",device)){strlcpy(botToken,device["telegram_bot_token"]|"",sizeof(botToken));strlcpy(chatId,device["telegram_chat_id"]|"",sizeof(chatId));}
  if(loadJson("/server.json",server)){JsonObject p=server["communication"]["telegram"];policyEnabled=p["enabled"]|true;sendReady=p["sendMachineReady"]|true;sendLossAlerts=p["sendLossAlerts"]|true;sendReports=p["sendReports"]|true;}
}
...
bool request(const String &url,String *bodyOut=nullptr){if(!WiFiManager::connected())return false;BearSSL::WiFiClientSecure client;client.setInsecure();client.setTimeout(10000);HTTPClient http;if(!http.begin(client,url))return false;http.setTimeout(10000);http.setFollowRedirects(HTTPC_STRICT_FOLLOW_REDIRECTS);const int code=http.GET();const String body=code>0?http.getString():String();http.end();if(bodyOut)*bodyOut=body;return code>=200&&code<300&&body.indexOf(F("\"ok\":true"))>=0;}
...
bool sendDocument(const String &path){if(!TelegramClient::configured()||!sendReports)return false;File file=LittleFS.open(path,"r");if(!file)return false;const String boundary=F("----AFMSBoundary7MA4YWxkTrZu0gW");String name=path;const int slash=name.lastIndexOf('/');if(slash>=0)name=name.substring(slash+1);String prefix=F("--");prefix+=boundary;prefix+=F("\r\nContent-Disposition: form-data; name=\"chat_id\"\r\n\r\n");prefix+=chatId;prefix+=F("\r\n--");prefix+=boundary;prefix+=F("\r\nContent-Disposition: form-data; name=\"document\"; filename=\"");prefix+=name;prefix+=F("\"\r\nContent-Type: text/csv\r\n\r\n");String suffix=F("\r\n--");suffix+=boundary;suffix+=F("--\r\n");BearSSL::WiFiClientSecure client;client.setInsecure();client.setTimeout(15000);if(!client.connect("api.telegram.org",443)){file.close();return false;}client.print(F("POST /bot"));client.print(botToken);client.println(F("/sendDocument HTTP/1.1"));client.println(F("Host: api.telegram.org"));client.print(F("Content-Type: multipart/form-data; boundary="));client.println(boundary);client.print(F("Content-Length: "));client.println(prefix.length()+file.size()+suffix.length());client.println(F("Connection: close\r\n"));client.print(prefix);uint8_t b[512];while(file.available()){const size_t n=file.read(b,sizeof(b));if(n)client.write(b,n);yield();}file.close();client.print(suffix);const uint32_t started=millis();while(!client.available()&&client.connected()&&millis()-started<15000UL)yield();String response;while(client.available())response+=char(client.read());client.stop();const bool ok=response.indexOf(F(" 200 "))>=0&&response.indexOf(F("\"ok\":true"))>=0;ok?++successCountValue:++failureCountValue;return ok;}
}
