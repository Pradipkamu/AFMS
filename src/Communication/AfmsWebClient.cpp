...
bool loadCredentials(String &baseUrl,String &apiKey){File file=LittleFS.open("/machine.json","r");if(!file)return false;DynamicJsonDocument doc(12288);const auto error=deserializeJson(doc,file);file.close();if(error)return false;baseUrl=doc["afms_server_url"]|"";apiKey=doc["device_api_key"]|"";return baseUrl.length()&&apiKey.length();}
...
bool sendPayload(const String &payload){String baseUrl,key;if(!loadCredentials(baseUrl,key))return false;WiFiClient client;HTTPClient http;const String url=baseUrl+"/api/v1/devices/telemetry";if(!http.begin(client,url))return false;http.addHeader("Content-Type","application/json");http.addHeader("X-AFMS-Device-Key",key);const int code=http.POST(payload);http.end();return code==200||code==201;}
void replayQueue(){if(!WiFiManager::connected()||static_cast<int32_t>(millis()-gNextReplayMs)<0)return;String eventId,payload;if(!OfflineQueue::peek(OfflineQueue::Destination::AfmsWeb,eventId,payload)){gReplayFailures=0;return;}if(sendPayload(payload)){OfflineQueue::removeHead(OfflineQueue::Destination::AfmsWeb);gReplayFailures=0;gNextReplayMs=millis()+1000UL;++gSuccess;Logger::info(F("[AFMS] Queued telemetry replayed"));}else{if(gReplayFailures<6)++gReplayFailures;uint32_t delayMs=5000UL<<gReplayFailures;if(delayMs>300000UL)delayMs=300000UL;gNextReplayMs=millis()+delayMs;++gFailure;}}
}
