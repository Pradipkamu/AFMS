...
bool readJson(const char *path,DynamicJsonDocument &doc){File file=LittleFS.open(path,"r");if(!file)return false;const auto error=deserializeJson(doc,file);file.close();return !error;}
bool loadCredentials(String &baseUrl,String &apiKey,String &fingerprint,bool &requireHttps){DynamicJsonDocument device(8192),server(12288);if(!readJson("/device.json",device))return false;readJson("/server.json",server);baseUrl=device["afms_server_url"]|"";apiKey=device["device_api_key"]|"";fingerprint=device["afms_tls_fingerprint"]|"";requireHttps=server["communication"]["afmsWeb"]["requireHttps"]|false;return baseUrl.length()&&apiKey.length();}
