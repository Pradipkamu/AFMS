...
app.post('/api/v1/telemetry', asyncHandler(async (req, res) => {
  ...
  if (lossCode || ['STOPPED', 'BREAKDOWN', 'IDLE'].includes(normalizedStatus)) {
    const code = lossCode || normalizedStatus;
    const open = await pool.query(`SELECT id FROM downtime_events WHERE machine_id=$1 AND status IN ('OPEN','ACKNOWLEDGED') ORDER BY started_at DESC LIMIT 1`, [machineId]);
    if (open.rowCount === 0) {
      const created = await pool.query(`INSERT INTO downtime_events(machine_id, loss_code, category, started_at) VALUES($1,$2,$3,COALESCE($4::timestamptz,NOW())) RETURNING *`, [machineId, code, normalizedStatus, recordedAt || null]);
      broadcast('downtime', created.rows[0]);
    }
  } else if (normalizedStatus === 'RUNNING') {
    const closed = await pool.query(`UPDATE downtime_events SET status='CLOSED', closed_at=COALESCE($2::timestamptz,NOW()) WHERE machine_id=$1 AND status IN ('OPEN','ACKNOWLEDGED') RETURNING *`, [machineId, recordedAt || null]);
    for (const row of closed.rows) broadcast('downtime', row);
  }
...
app.get('/api/v1/machines/latest', asyncHandler(async (_req, res) => {
  const result = await pool.query(`SELECT DISTINCT ON (machine_id) machine_id AS "machineId", status, production_count AS "productionCount", reject_count AS "rejectCount", cycle_time_ms AS "cycleTimeMs", loss_code AS "lossCode", recorded_at AS "recordedAt" FROM machine_telemetry ORDER BY machine_id, recorded_at DESC, id DESC`);
  res.json(result.rows);
}));

app.get('/api/v1/downtime', asyncHandler(async (req, res) => {
  const status = String(req.query.status || '').toUpperCase();
  const machineId = String(req.query.machineId || '');
  const limit = Math.min(500, Math.max(1, Number(req.query.limit) || 100));
  const result = await pool.query(`
    SELECT id, machine_id AS "machineId", loss_code AS "lossCode", category, status,
      started_at AS "startedAt", acknowledged_at AS "acknowledgedAt",
      acknowledged_by AS "acknowledgedBy", note, closed_at AS "closedAt",
      EXTRACT(EPOCH FROM (COALESCE(closed_at, NOW()) - started_at))::bigint AS "durationSeconds"
    FROM downtime_events
    WHERE ($1='' OR status=$1) AND ($2='' OR machine_id=$2)
    ORDER BY started_at DESC LIMIT $3`, [status, machineId, limit]);
  res.json(result.rows);
}));

app.post('/api/v1/downtime', asyncHandler(async (req, res) => {
  const { machineId, lossCode, category = 'MANUAL', startedAt, note = null } = req.body;
  if (!machineId || !lossCode) return res.status(400).json({ error: 'machineId and lossCode are required' });
  const result = await pool.query(`INSERT INTO downtime_events(machine_id,loss_code,category,started_at,note) VALUES($1,$2,$3,COALESCE($4::timestamptz,NOW()),$5) RETURNING *`, [machineId, lossCode, category, startedAt || null, note]);
  broadcast('downtime', result.rows[0]);
  res.status(201).json(result.rows[0]);
}));

app.post('/api/v1/downtime/:id/acknowledge', asyncHandler(async (req, res) => {
  const { acknowledgedBy = 'Supervisor', note = null, category = null } = req.body;
  const result = await pool.query(`UPDATE downtime_events SET status='ACKNOWLEDGED', acknowledged_at=NOW(), acknowledged_by=$2, note=COALESCE($3,note), category=COALESCE($4,category) WHERE id=$1 AND status='OPEN' RETURNING *`, [req.params.id, acknowledgedBy, note, category]);
  if (!result.rowCount) return res.status(404).json({ error: 'Open downtime event not found' });
  broadcast('downtime', result.rows[0]);
  res.json(result.rows[0]);
}));

app.post('/api/v1/downtime/:id/close', asyncHandler(async (req, res) => {
  const result = await pool.query(`UPDATE downtime_events SET status='CLOSED', closed_at=NOW(), note=COALESCE($2,note) WHERE id=$1 AND status<>'CLOSED' RETURNING *`, [req.params.id, req.body.note || null]);
  if (!result.rowCount) return res.status(404).json({ error: 'Active downtime event not found' });
  broadcast('downtime', result.rows[0]);
  res.json(result.rows[0]);
}));
