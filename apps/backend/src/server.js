...
async function ensureSchema() {
  await pool.query(`CREATE TABLE IF NOT EXISTS downtime_events (id BIGSERIAL PRIMARY KEY,machine_id TEXT NOT NULL,loss_code TEXT NOT NULL,category TEXT NOT NULL DEFAULT 'UNCLASSIFIED',status TEXT NOT NULL DEFAULT 'OPEN' CHECK (status IN ('OPEN','ACKNOWLEDGED','CLOSED')),started_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),acknowledged_at TIMESTAMPTZ,acknowledged_by TEXT,note TEXT,closed_at TIMESTAMPTZ,created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()); CREATE INDEX IF NOT EXISTS idx_downtime_machine_status ON downtime_events (machine_id,status,started_at DESC); CREATE INDEX IF NOT EXISTS idx_downtime_started_at ON downtime_events (started_at DESC);`);
}
...
app.post('/api/v1/telemetry',asyncHandler(async(req,res)=>{
  ...
  const result=await pool.query(`INSERT INTO machine_telemetry(machine_id,status,production_count,reject_count,cycle_time_ms,loss_code,recorded_at) VALUES($1,$2,$3,$4,$5,$6,COALESCE($7::timestamptz,NOW())) RETURNING id,machine_id AS "machineId",status,production_count AS "productionCount",reject_count AS "rejectCount",cycle_time_ms AS "cycleTimeMs",loss_code AS "lossCode",recorded_at AS "recordedAt"`,[machineId,status,productionCount,rejectCount,cycleTimeMs,lossCode,recordedAt||null]);
  const telemetry=result.rows[0], normalizedStatus=String(status).toUpperCase();
  if(lossCode||['STOPPED','BREAKDOWN','IDLE'].includes(normalizedStatus)){const code=lossCode||normalizedStatus;const open=await pool.query(`SELECT id FROM downtime_events WHERE machine_id=$1 AND status IN ('OPEN','ACKNOWLEDGED') ORDER BY started_at DESC LIMIT 1`,[machineId]);if(!open.rowCount){const created=await pool.query(`INSERT INTO downtime_events(machine_id,loss_code,category,started_at) VALUES($1,$2,$3,COALESCE($4::timestamptz,NOW())) RETURNING *`,[machineId,code,normalizedStatus,recordedAt||null]);broadcast('downtime',created.rows[0]);}}
  else if(normalizedStatus==='RUNNING'){const closed=await pool.query(`UPDATE downtime_events SET status='CLOSED',closed_at=COALESCE($2::timestamptz,NOW()) WHERE machine_id=$1 AND status IN ('OPEN','ACKNOWLEDGED') RETURNING *`,[machineId,recordedAt||null]);for(const row of closed.rows)broadcast('downtime',row);}
  broadcast('telemetry',telemetry); return res.status(201).json(telemetry);
}));

app.get('/api/v1/machines/latest',asyncHandler(async(_req,res)=>{const result=await pool.query(`SELECT DISTINCT ON (machine_id) machine_id AS "machineId",status,production_count AS "productionCount",reject_count AS "rejectCount",cycle_time_ms AS "cycleTimeMs",loss_code AS "lossCode",recorded_at AS "recordedAt" FROM machine_telemetry ORDER BY machine_id,recorded_at DESC,id DESC`);res.json(result.rows);}));
